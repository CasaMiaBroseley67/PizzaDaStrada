...
app.get('/api/export-pdf', async (req, res) => {
    ...
    res.setHeader('Content-Disposition', 'attachment; filename=Pizza Da Strada_SFBB_Log.pdf');
    doc.pipe(res);

    doc.fontSize(22).font('Helvetica-Bold').text('Pizza Da Strada SFBB Compliance Log', { align: 'center' });
...
app.get('/api/export-allergen-pdf', async (req, res) => {
    ...
    res.setHeader('Content-Type', 'application/pdf');
    res.setHeader('Content-Disposition', 'attachment; filename=Pizza Da Strada_Allergens.pdf');
    doc.pipe(res);
    doc.fontSize(18).font('Helvetica-Bold').text('DISHES AND THEIR ALLERGEN CONTENT - Pizza Da Strada', { align: 'left' });
    doc.moveDown(2);
...
app.get('/api/export-risk-pdf', async (req, res) => {
    const doc = new PDFDocument({ margin: 30, size: 'A4', layout: 'landscape' });
    res.setHeader('Content-Type', 'application/pdf');
    res.setHeader('Content-Disposition', 'attachment; filename=Pizza Da Strada_Risks.pdf');
    doc.pipe(res); doc.fontSize(18).text('Risk Assessment Template - Pizza Da Strada'); doc.end();
});
...
app.get('/api/export-docs-pdf', async (req, res) => {
    ...
    const doc = new PDFDocument({ margin: 50 });
    res.setHeader('Content-Type', 'application/pdf');
    res.setHeader('Content-Disposition', 'attachment; filename=Pizza Da Strada_Certificates.pdf');
    doc.pipe(res);

    doc.fontSize(22).font('Helvetica-Bold').text('Pizza Da Strada Compliance Certificates', { align: 'center' });
    doc.moveDown(2);
